 ...
 smd(
  ...
  async message => {
   try {
    ...
    const balance = await eco.balance(users, 'Asta')
    await eco.deduct(users, 'Asta', balance.wallet)
   ...
   try {
    ...
    switch (value) {
     ...
     case '1':
      ...
      const deduct1 = await eco.deduct(user, 'Asta', 100)
      const add1 = eco.giveCapacity(user, 'Asta', 1000)
     ...
     case '2':
      ...
      const deduct2 = await eco.deduct(user, 'Asta', 1000)
      const add2 = eco.giveCapacity(user, 'Asta', 100000)
     ...
     case '3':
      ...
      const deduct3 = await eco.deduct(user, 'Asta', 10000)
      const add3 = eco.giveCapacity(user, 'Asta', 10000000)
  ...
  async (message, match) => {
   try {
    ...
    return await message.bot.sendMessage(
     message.chat,
     { text: `Added 📈 ${parseInt(match.split(' ')[0])} to @${users.split('@')[0]} wallet🛸.`, mentions: [users] },
     { quoted: message }
    )
   } catch (e) {
    message.error(`${e}\n\ncommand: give`, e)
