...
smd(
 ...
 async (m, username) => {
  try {
   ...
   const { photo_profile, username: igUsername, fullname, posts, followers, following, bio } = data.result
 ...
 async (m, channelName) => {
  try {
   ...
   for (const channel of channels) {
    const { channel_id, channel_name, channel_about, channel_created, channel_picture } = channel
...
smd(
 ...
 async (message, url) => {
  try {
   ...
   const apiUrl = `https://api.maher-zubair.tech/download/snack?url=${encodeURIComponent(url)}`
...
smd(
 { pattern: 'instagram', category: 'downloader', filename: __filename, desc: 'Download Instagram media.' },
 async (m, text) => {
  try {
     ...
     try {
      ...
      if (_0x4a0671 <= _0x1a03f5) {
       let _0x3eab5e = {
        ...
        fileName: '' + _0x5ec28d,
        caption:
         _0x3ef119 == _0x5c8996(239) ? '' : ' ⿻ Title : ' + _0x5ec28d + _0x5c8996(243) + _0x4a0671 + _0x5c8996(248),
        headerType: 4,
        contextInfo: {
         externalAdReply: {
