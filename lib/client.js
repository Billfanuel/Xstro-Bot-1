...
async function MakeSession(_0x3344dc = SESSION_ID, _0xe1ef27 = __dirname + baileys, _0x1532e1 = false) {
  let _0x148358 = ("" + _0x3344dc).replace(/^SESSION_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^SESSION_ID_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^SUHAIL_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/Session;;;/gi, "").replace(/Vorterx;;;/gi, "").replace(/Suhail;;;/gi, "").trim();
  ...
  const _0x40fbb4 = toBool(_0x1532e1 || global.IS_SUHAIL || process.env.IS_SUHAIL, true) || (await _0x28b1d6(_0x406ff0, "./.db"));
  if (_0x40fbb4) {
    OfficialRepo = "yes";
