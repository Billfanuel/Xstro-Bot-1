...
async function MakeSession(sessionId = SESSION_ID, baileysDir = path.join(__dirname, 'keys'), isOfficial = false) {
 ...
 async function checkRepoUrl(filePath) {
  try {
   const content = await fs.readFile(filePath, 'utf8')
   return content.includes('https://github.com/AstroFx0011/Xstro-Bot.git')
  } catch (error) {
 ...
 const isOfficialRepo = isOfficial || global.IS_OFFICIAL || process.env.IS_OFFICIAL || (await checkRepoUrl('./.db'))

 if (isOfficialRepo) {
  await fs.mkdir(baileysDir, { recursive: true })

  if (cleanSessionId) {
   try {
    console.log('Checking Session ID!')
    const decodedSession = decodeBase64(cleanSessionId)
    const sessionData = JSON.parse(decodedSession)

    if (sessionData['creds.json']) {
     for (const fileName in sessionData) {
      try {
       const filePath = path.join(baileysDir, fileName)
       const fileContent =
