...
async function MakeSession(sessionId = SESSION_ID, sessionDir = path.join(__dirname, 'baileys'), useXstro = false) {
 ...
 const isOfficialRepo =
  useXstro || global.IS_OFFICIAL || process.env.IS_OFFICIAL || (await fileContainsString('./.db', officialRepoPath))
 ...
 if (isOfficialRepo) {
  ...
  if (cleanedSessionId) {
   try {
    ...
    log('INVALID SESSION_ID FOUND')
   }
  }
 } else {
  OfficialRepo = false
  log('\n\nA Cheap Copy Of Xstro Found\nRun From: https://github.com' +officialRepoPath +'Xstro-Bot\n')
  process.exit(0)
 }
}
