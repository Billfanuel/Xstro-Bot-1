...
const { ping, Cpu } = require('./Base/class.js')
...
async function Draw(prompt) {
 ...
 if (jsonResponse.status === 200) {
  const imageUrl = jsonResponse.result
  const imageResponse = await fetch(imageUrl)
  const imageBuffer = await imageResponse.buffer() // Using buffer() to get image as Buffer
